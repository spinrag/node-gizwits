...
const debug = require('debug')('clearlight:device');
const VarintBufferReader = require('./varint-buffer-reader');

const DEFAULT_TCP_PORT = 12416;
 ...
 */
class ClearlightDevice extends EventEmitter {
    ...
    connect() {
        return new Promise((resolve, reject) => {
            ...
            const port = this.options.port || DEFAULT_TCP_PORT;
        ...
        this.expectedResponsePromises[responseType] = {
            ...
            responseTimeout: setTimeout(() => {
                if (this.expectedResponsePromises[responseType]) {
                    ...
                    this.expectedResponsePromises[responseType].responsePromise.reject(new Error(`Response timeout for type ${responseType}`));
    ...
    async setAttribute(attributes) {
        // Create the base command structure
        const payload = Buffer.alloc(13);  // Length is always 13 for flag commands
        payload[0] = 0x01;  // Command type        
        payload[1] = 0x00;  // Always 0 for flags

        if ('EXTERNAL_LIGHT' in attributes) {
            payload[2] = 0x01;  // Attribute ID for EXTERNAL_LIGHT
            payload[3] = attributes.EXTERNAL_LIGHT ? 0x01 : 0x00;
        } else if ('INTERNAL_LIGHT' in attributes) {
            payload[2] = 0x02;  // Attribute ID for INTERNAL_LIGHT
            payload[3] = attributes.INTERNAL_LIGHT ? 0x02 : 0x00;
        } else if ('PRE_TIME_FLAG' in attributes) {
            payload[2] = 0x04;  // Attribute ID for PRE_TIME_FLAG
            payload[3] = attributes.PRE_TIME_FLAG ? 0x04 : 0x00;
        } else if ('power_flag' in attributes) {
            payload[2] = 0x08;  // Attribute ID for power_flag
            payload[3] = attributes.power_flag ? 0x08 : 0x00;
        } else if ('cf_flag' in attributes) {
            payload[2] = 0x10;  // Attribute ID for cf_flag
            payload[3] = attributes.cf_flag ? 0x10 : 0x00;
        } else if ('UVB_FLAG' in attributes) {
            payload[2] = 0x20;  // Attribute ID for UVB_FLAG
            payload[3] = attributes.UVB_FLAG ? 0x20 : 0x00;
        } else if ('N_FLAG' in attributes) {
            payload[2] = 0x40;  // Attribute ID for N_FLAG
            payload[3] = attributes.N_FLAG ? 0x40 : 0x00;
        } else if ('RIGHT' in attributes) {
            payload[1] = 0x03;  // Command type for SPECTRUM
            payload[5] = attributes.RIGHT;
            payload[6] = this._lastData?.LEFT || attributes.RIGHT;
        } else if ('LEFT' in attributes) {
            payload[1] = 0x03;  // Command type for SPECTRUM
            payload[5] = this._lastData?.RIGHT || attributes.LEFT;
            payload[6] = attributes.LEFT;
        } else if ('SET_TEMP' in attributes) {
            payload[1] = 0x04;  // Command type for SET_TEMP
            payload[7] = attributes.SET_TEMP;  // Temperature value
        } else if ('SET_HOUR' in attributes) {
            payload[1] = 0x08;  // Command type for SET_HOUR
            payload[8] = attributes.SET_HOUR;
        } else if ('SET_MINUTE' in attributes) {
            payload[1] = 0x10;  // Command type for SET_MINUTE
            payload[9] = attributes.SET_MINUTE;
        } else if ('PRE_TIME_HOUR' in attributes) {
            payload[1] = 0x20;  // Command type for PRE_TIME_HOUR
            payload[10] = attributes.PRE_TIME_HOUR;
        } else if ('PRE_TIME_MINUTE' in attributes) {
            payload[1] = 0x40;  // Command type for PRE_TIME_MINUTE
            payload[11] = attributes.PRE_TIME_MINUTE;
        }
    
        // Fill only unset bytes with 0
        for (let i = 4; i < 13; i++) {
        ...
        debug(`Setting attribute: ${JSON.stringify(attributes)}`);
        debug(`Control payload bytes: [${[...payload].map(b => '0x' + b.toString(16).padStart(2, '0')).join(', ')}]`);
    
        return this.sendControl(payload);
    }
    ...
    async sendControl(payload) {
        ...
        this._sequenceNumber = (this._sequenceNumber || 0) + 1;
        seqNum.writeUInt32BE(this._sequenceNumber);
    
        // Format matches your examples exactly:
        // 00 00 00 03 14 00 00 93 [sequence] [payload]
        const headerBuffer = Buffer.from([0x00, 0x00, 0x00, 0x03, 0x14, 0x00, 0x00, 0x93]);
        const fullBuffer = Buffer.concat([headerBuffer, seqNum, payload]);
    
        debug(`Full control command bytes: [${[...fullBuffer].map(b => '0x' + b.toString(16).padStart(2, '0')).join(', ')}]`);
        
        try {
            const result = await this._sendAndRegisterForResponse(fullBuffer, 0x94);
            debug('Raw control command response:', result);
            return result;
        } catch (error) {
            debug('Control command error:', error);
            const errorMessage = error instanceof Error ? error.message : (error ? String(error) : 'Unknown error');
            throw new Error(`Control command failed: ${errorMessage}`);
        }
    }    
    ...
    _handleData(message) {
        ...
        switch (messageType) {
            ...
            case 0x91:
                ...
                break;
            case 0x94:
                console.log('_handleDataExtendedResponse')
    ...
    _handleLoginResponse(data) {
        ...
        if (dataReader.nextInt8() === 0x00) {
            ...
            this.pingTimeout = setTimeout(() => {
                this.pingTimeout = null;
                this._sendPing();
            }, this.options.pingInterval || PING_INTERVAL);
        } else {
            debug('login failed');
    ...
    _handleDataResponse(data) {
        ...
        // First byte contains boolean flags
        const flagsByte = dataReader.nextInt8();
        const flagsByte2 = dataReader.nextInt8();
        debug(`Flags byte: ${flagsByte.toString(2).padStart(8, '0')}`); // Add this debug line
        debug(`Flags byte: ${flagsByte2.toString(2).padStart(8, '0')}`); // Add this debug line
        const res = {
            // Boolean flags from first byte - verify these bit positions
            EXTERNAL_LIGHT: !!(flagsByte2 & 0x01),  // bit 0 (most significant bit)
            INTERNAL_LIGHT: !!(flagsByte2 & 0x02),  // bit 1
            PRE_TIME_FLAG: !!(flagsByte2 & 0x04),   // bit 2
            power_flag: !!(flagsByte2 & 0x08),      // bit 3
            cf_flag: !!(flagsByte2 & 0x10),         // bit 4
            UVB_FLAG: !!(flagsByte2 & 0x20),        // bit 5  // AUX Control
            N_FLAG: !!(flagsByte2 & 0x40),          // bit 6
    
            // Remaining uint8 values stay the same
            LED: dataReader.nextUInt8(),
            RIGHT: dataReader.nextUInt8(),
            LEFT: dataReader.nextUInt8(),
            SET_TEMP: dataReader.nextUInt8(),
            SET_HOUR: dataReader.nextUInt8(),
            SET_MINUTE: dataReader.nextUInt8(),
            PRE_TIME_HOUR: dataReader.nextUInt8(),
            PRE_TIME_MINUTE: dataReader.nextUInt8(),
            SN: dataReader.nextUInt8(),
            CURRENT_TEMP: dataReader.nextUInt8(),
            heart_pulse: dataReader.nextUInt8()
        };

        // debug(`Retrieved data: ${JSON.stringify(res, null, 2)}`);
        this._lastData = res;
        this.emit('data', res);
    ...
    _handleDataExtendedResponse(data) {
        debug('_handleDataExtendedResponse received raw data:', data.toString('hex'));
        // const dataReader = new VarintBufferReader(data);
        ...
        // };
        // debug(`Extended Retrieved data: ${JSON.stringify(res)}`);
        // this.emit('data', res);
        // this._resolveResponse(0x94, { success: true, sequenceNumber: sn });
        // this._resolveResponse(0x94, res);
        this._resolveResponse(0x94, { 
            success: true,
    ...
    _sendPing() {
        ...
        this.pingWaitTimeout = setTimeout(() => {
            ...
            this.close(true);
        }, (this.options.pingInterval || PING_INTERVAL) * 2);
    }
}

module.exports = ClearlightDevice;
